...
private:
    ...
    std::pair<std::string,std::string> mCurrentStatusObjectState; 
    ...
    /**
     * Creates a JSON Object with the current output of the status of the given object
     * @param object: Object that inherits from Status and whose state is being printed.
     */
    nlohmann::json getStatus(Status *object);

    static nlohmann::json getResult(const parameterCombination &cords, functionValue optimum);

    
public:
    
    
    static nlohmann::json getResults(
    ...
    std::vector<nlohmann::json> updateStatusValues(Status *opt, Status *runner, Status *eval,
