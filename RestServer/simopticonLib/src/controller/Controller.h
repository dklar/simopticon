...
private:
    ...
    std::function<void(std::string)> mDataSendCallback;
    ...
    nlohmann::json packData(const std::string& type, const nlohmann::json& data);
 ...
 */
    ...
    // void createCsvData(const std::filesystem::path& path);
    // void createPlots(const std::filesystem::path &path,const std::string& argument);
...
public:
    ...
    explicit Controller(const std::filesystem::path &configPath,std::function<void(std::string)> callback,std::function<void(std::string)> dataCallback, bool isStub = false);
    ...
    void sendData(const nlohmann::json& dataMessage);

    void sendBinary(const std::filesystem::path& path);

    /**
     * Sends the actual JSON data to a callback
     * @param jsonSendObject A JSON string representing a JSON Object
    */
    void send(const std::string& jsonSendObject);
